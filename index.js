// FoodBridge — LLM-first shared-plan MVP (no localStorage)
...
const plans = new Map(); // planId -> { recipes: [], createdAt, ttlAt }
function newPlan() { const id = crypto.randomUUID(); const now = Date.now(); plans.set(id, { recipes: [], createdAt: now, ttlAt: now + TTL_MS }); return id; }
function getPlan(id) { const p = plans.get(id); if (!p) return null; if (Date.now() > p.ttlAt) { plans.delete(id); return null; } return p; }
setInterval(() => { const now = Date.now(); for (const [id, p] of plans.entries()) if (now > p.ttlAt) plans.delete(id); }, 60_000);

// ===== Minimal corpus fetcher (stub) =====
async function fetchCorpusRecipeById(corpusId) {
  return { title: `Corpus Recipe ${corpusId}`, text: `Ingredients: 2 eggs; 1 cup flour; 1 tsp salt.\nSteps: Mix; bake 15m.` };
}

// ===== LLM normalizer =====
async function llmNormalizeRecipe({ text, url }) {
  const prompt = `
...
app.post("/api/plan/:id/recipes/text", async (req, res) => {
  ...
  plan.recipes.push({ ...normalized, ingredients: priced }); plan.ttlAt = Date.now() + TTL_MS; res.json({ ok: true, plan });
});
...
app.post("/api/plan/:id/recipes/url", async (req, res) => {
  ...
  const normalized = await llmNormalizeRecipe({ text: html, url }); const priced = priceIngredients(normalized.ingredients);
  plan.recipes.push({ ...normalized, ingredients: priced, source: url }); plan.ttlAt = Date.now() + TTL_MS; res.json({ ok: true, plan });
});
...
app.post("/api/plan/:id/recipes/corpus", async (req, res) => {
  ...
  for (const corpusId of ids) {
    const r = await fetchCorpusRecipeById(corpusId);
    const normalized = await llmNormalizeRecipe({ text: r.text }); const priced = priceIngredients(normalized.ingredients);
    plan.recipes.push({ ...normalized, ingredients: priced, corpusId });
  }
  plan.ttlAt = Date.now() + TTL_MS; res.json({ ok: true, plan });
});

app.post("/api/plan/:id/email", async (req, res) => {
  const plan = getPlan(req.params.id); if (!plan) return res.status(404).json({ error: "Plan not found/expired" });
  const { to } = req.body; if (!to) return res.status(400).json({ error: "to required" });
  if (!RESEND_API_KEY) return res.status(400).json({ error: "RESEND_API_KEY missing" });

  const total = plan.recipes.flatMap(r => r.ingredients||[]).reduce((s,i)=>s+(Number(i.price)||0),0);
  const html = `
    <div style="font-family:Arial,sans-serif">
      <h2>FoodBridge Plan</h2>
      ${plan.recipes.map(r => `
        <h3>${r.title}</h3>
        <strong>Ingredients</strong>
        <ul>${(r.ingredients||[]).map(i => `<li>${i.quantity??""} ${i.unit??""} ${i.name} — $${(i.price??0).toFixed(2)}</li>`).join("")}</ul>
        <strong>Steps</strong>
        <ol>${(r.steps||[]).map(s => `<li>${s}</li>`).join("")}</ol>
      `).join("")}
      <hr/><p><strong>Estimated Total:</strong> $${total.toFixed(2)}</p>
      <p>Share link: <a href="\${req.protocol}://\${req.get("host")}/p/\${req.params.id}">\${req.protocol}://\${req.get("host")}/p/\${req.params.id}</a></p>
    </div>
  `;
  ...
  const resp = await fetch("https://api.resend.com/emails", {
    ...
    body: JSON.stringify({ from: "FoodBridge <onboarding@resend.dev>", to: [to], subject: "Your FoodBridge Plan", html })
  });
  const data = await resp.json(); res.json({ ok: true, id: data?.id || null });
});

...
${text}`.trim();
  ...
  if (!OPENAI_API_KEY) {
    return {
      ...
      steps: ["Mix things", "Cook until done"],
    };
  }
...
function mockPrice(name) {
  const h = crypto.createHash("md5").update((name || "").toLowerCase()).digest("hex");
  const n = parseInt(h.slice(0, 6), 16) % 700;
  return Number((0.99 + n / 100).toFixed(2));
}
function priceIngredients(ingredients) {
  return (ingredients || []).map(i => ({ ...i, price: mockPrice(i.name || "item") }));
}
...
app.post("/api/plan/:id/recipes/url", async (req, res) => {
  ...
  if (!r || !r.ok) return res.status(400).json({ error: "Failed to fetch URL" });
  const html = await r.text();
  const normalized = await llmNormalizeRecipe({ text: html });
  plan.recipes.push({ ...normalized, source: url, ingredients: priceIngredients(normalized.ingredients) });
  plan.ttlAt = nowMs() + TTL_MS;
  res.json({ ok: true, plan });
});
...
app.post("/api/plan/:id/recipes/corpus", async (req, res) => {
  ...
  for (const cid of ids) {
    ...
    const normalized = await llmNormalizeRecipe({ text });
    plan.recipes.push({ ...normalized, corpusId: cid, ingredients: priceIngredients(normalized.ingredients) });
  }
  plan.ttlAt = nowMs() + TTL_MS;
  res.json({ ok: true, plan });
});
...
app.get("/p/:id", (req, res) => {
  const plan = getPlan(req.params.id);
  if (!plan) return res.status(404).send("Plan not found/expired");
...
// ===== Compatibility routes for your current front-end =====
// 1) /api/health alias
app.get("/api/health", (_, res) => res.json({ ok: true }));

// 2) POST /api/ingest/free-text  (creates a plan, adds the text recipe, returns it)
...
// 3) GET /api/ingredients/suggest?q=... (simple mock)

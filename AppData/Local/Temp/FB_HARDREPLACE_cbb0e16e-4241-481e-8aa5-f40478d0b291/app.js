(() => {
  ...
  document.getElementById("btn-suggest")?.addEventListener("click", async ()=>{
    ...
    try{
      ...
      ul.innerHTML=(data.suggestions||[]).map(s=>`<li>${escapeHtml(s.name)} <button class="btn" data-suggest="${encodeURIComponent(s.name)}">+</button></li>`).join("")||"<li>(no results)</li>";
    } finally { showSpinner(false); }
  });
  ...
  function renderRecipeReadout(prefix, r){
    ...
    if (titleEl) titleEl.textContent = r.title || "Recipe";
    if (metaEl)  metaEl.textContent  = "Servings: " + (r.servings ?? 4);
    if (ingEl)   ingEl.innerHTML     = (r.ingredients||[]).map(i=>`<li>${escapeHtml(i.name||i)}</li>`).join("");
    if (stepsEl) stepsEl.innerHTML   = (r.steps||[]).map(s=>`<li>${escapeHtml(s)}</li>`).join("");
  }
})();
